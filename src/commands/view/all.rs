...
async fn view_all(ctx: AppContext<'_>, pages: Vec<Position>) -> Result<(), serenity::Error> {
    ...
    let reply = {
        ...
        poise::CreateReply::default()
            .embed(
                serenity::CreateEmbed::default().description(
                    label_display(
                        0,
                        pages.len() as u32,
                        &format!("{}Status: `{}`", pages[0].display(), pages[0].get_status()),
                    )
                    .await,
                ),
            )
